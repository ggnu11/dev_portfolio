...
export const projects = [
  ...
  },
  {
    ...
    sub_title: { kr: "React 19 + Firebase 서버리스 프로젝트 대시보드", jp: "React 19 + Firebaseサーバーレスプロジェクトダッシュボード" },
    period: { kr: "2025.06 - 현재", jp: "2025.06 - 現在" }, member: "Solo Project", skills: [] as string[],
    links: [{ href: "https://ggnu11.github.io/site-dash/", label: "Live Demo" }],
    skill_ids: [4, 5, 7, 9, 13, 14, 23, 25, 30], row_number: 5,
    items: [
      { id: 5, title: { kr: "인증 및 아키텍처", jp: "認証及びアーキテクチャ" }, content: [
        { kr: "Firebase Auth + Google OAuth 2.0 소셜 로그인 및 회원탈퇴 구현", jp: "Firebase Auth + Google OAuth 2.0ソーシャルログイン及び退会実装" },
        { kr: "FSD (Feature-Sliced Design) 아키텍처로 확장 가능한 구조 설계", jp: "FSD（Feature-Sliced Design）アーキテクチャで拡張可能な構造設計" },
        { kr: "Firebase Firestore 기반 프로젝트·서브메뉴 CRUD 및 사용자별 데이터 격리", jp: "Firebase Firestoreベースのプロジェクト・サブメニューCRUD及びユーザー別データ分離" },
        { kr: "ProtectedRoute 기반 인증 상태 라우팅 가드 구현", jp: "ProtectedRouteベースの認証状態ルーティングガード実装" },
      ], projectId: 3, blobUrl: null, row_number: 1, image_ratio: null },
      { id: 6, title: { kr: "UI/UX 및 배포", jp: "UI/UX及びデプロイ" }, content: [
        { kr: "Framer Motion 타이핑 텍스트·캐러셀·시네마틱 전환 애니메이션 구현", jp: "Framer Motionタイピングテキスト・カルーセル・シネマティック遷移アニメーション実装" },
        { kr: "Zustand 기반 메뉴·인증 상태 관리 + Sonner 토스트 알림 연동", jp: "Zustandベースのメニュー・認証状態管理 + Sonnerトースト通知連携" },
        { kr: "shadcn/ui (Radix UI) + Tailwind CSS v4 다크 테마 UI 구현", jp: "shadcn/ui（Radix UI）+ Tailwind CSS v4ダークテーマUI実装" },
        { kr: "GitHub Pages 배포 (Firebase 서버리스 백엔드)", jp: "GitHub Pagesデプロイ（Firebaseサーバーレスバックエンド）" },
